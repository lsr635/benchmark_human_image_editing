...
# ======================================================================
    ...
    if cfg.args.dataset_path is not None:
        ...
        if not os.path.isdir(dataset_case_dir):
            raise FileNotFoundError(f"case_id `{case_id}` does not exist under `{dataset_root}`.")

        def _candidate(path_name):
            candidate = os.path.join(dataset_case_dir, path_name)
            return candidate if os.path.exists(candidate) else None

        if cfg.args.input_image is None:
            cfg.args.input_image = _candidate("input.png")
        if cfg.args.input_mask is None:
            cfg.args.input_mask = _candidate("mask.png")
        if cfg.args.input_reference_image is None:
            cfg.args.input_reference_image = _candidate("subject.png")
        if cfg.args.instruction == "":
            prompt_path = _candidate("prompt.txt")
            if prompt_path is not None:
                with open(prompt_path, "r", encoding="utf-8") as prompt_file:
                    cfg.args.instruction = prompt_file.read().strip()
        if cfg.args.output_dir is not None:
            os.makedirs(cfg.args.output_dir, exist_ok=True)
            if cfg.args.save_path == 'examples/output_images/output.png' or not cfg.args.save_path:
                cfg.args.save_path = os.path.join(cfg.args.output_dir, f"{case_id}.png")
...
# ===============================================================================
    if cfg.args.instruction == "" and cfg.args.input_image is None and cfg.args.input_reference_image is None:
        ...
        params = {
            ...
            "model_path": FS.get_from(task_model_dict[task_key]["MODEL_PATH"])
        }
