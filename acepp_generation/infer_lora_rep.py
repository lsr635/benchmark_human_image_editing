...
# ======================================================================
    ...
    if cfg.args.dataset_path is not None:
        ...
        def _build_case(case_id):
            ...
            case_inputs = {
                "input_image": _candidate(case_dir, "input.png", "rep.png"),
                "input_mask": _candidate(case_dir, "mask.png", "label_mask.png"),
                "input_reference_image": _candidate(case_dir, "subject.png", "token_rep.png", "token.png"),
            }
            ...
            if not instruction_value:
                prompt_path = _candidate(case_dir, "prompt.txt", "prompt_replacement_qwen.txt", "prompt_rep.txt")
                if prompt_path is not None:
                    with open(prompt_path, "r", encoding="utf-8") as prompt_file:
                        instruction_value = prompt_file.read().strip()
            if not instruction_value:
                meta_path = _candidate(case_dir, "meta.json")
                if meta_path is not None:
                    with open(meta_path, "r", encoding="utf-8") as meta_file:
                        meta_data = json.load(meta_file)
                    instruction_value = (
                        meta_data.get("prompt")
                        or meta_data.get("target_prompt")
                        or meta_data.get("prompt_used")
                        or ""
                    )
...
# ===============================================================================
    ...
    if run_examples:
        ...
        resolved_save_path = cfg.args.save_path
        if not resolved_save_path and dataset_case_dir is not None:
            resolved_save_path = case_inputs.get("save_path")
        if not resolved_save_path:
            resolved_save_path = "examples/output_images/output.png"
